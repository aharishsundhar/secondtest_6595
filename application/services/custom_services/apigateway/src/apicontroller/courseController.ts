...
export class courseController implements Controller {
    ...
    private initializeRoutes() {
        this.router.delete('/course/:id', this.GpDelete);
this.router.get('/course/get/search', this.GpSearch);
this.router.put('/course/get/update', this.GpSearchForUpdate);
this.router.put('/course', this.GpUpdate);
this.router.get('/course', this.GpGetAllValues);
this.router.post('/course', this.GpCreate);
this.router.get('/course/:id', this.GpGetNounById);
    }

public GpDelete(req: Request, res: Response) {
            new CustomLogger().showLogger('info', 'Enter into courseController.ts: GpDelete');
        new ApiAdaptar().delete(Constant.COURSESURL + `${req.url}` )
...
public GpSearch(req: Request, res: Response) {
        ...
        new ApiAdaptar().get(Constant.COURSESURL + `${req.url}` )
        .then((res: any) => res.response.json()).then(result => {
              req.baseUrl === '/mobile' ? res.send(result) :
              req.baseUrl === '/desktop' ? res.send(result) : res.send(null)
...
public GpSearchForUpdate(req: Request, res: Response) {
        ...
        new ApiAdaptar().put(Constant.COURSESURL + `${req.url}` , req.body)
        .then((res: any) => res.response.json()).then(result => {
              req.baseUrl === '/mobile' ? res.send(result) :
              req.baseUrl === '/desktop' ? res.send(result) : res.send(null)
...
public GpCreate(req: Request, res: Response) {
        ...
        new ApiAdaptar().post(Constant.COURSESURL + `${req.url}` , req.body)
        .then((res: any) => res.response.json()).then(result => {
              req.baseUrl === '/mobile' ? res.send(result) :
              req.baseUrl === '/desktop' ? res.send(result) : res.send(null)
